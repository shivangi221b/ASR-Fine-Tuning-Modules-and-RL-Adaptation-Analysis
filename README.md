...
**Useful flags:** `--use_lora` (best-effort NeMo encoder adapter), `--mock_llm` / `--real_llm`, `--skip_zero_shot`, `--skip_librispeech_forgetting`, `--skip_test_eval`, `--voxpopuli_train_subset N` (optional cap; omitted = full VoxPopuli train split).
